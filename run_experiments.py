...
def build_template(base: BaseConfig) -> None:
    ...
    if premier.is_sealed(TEMPLATE):
        raise SystemExit(
            ...
            f"domain '{domain.active().name}' ({premier.expected()}), the base of "
            f"premier '{premier.PREMIER_NAME}'.\n"
...
def prepare_experiment(name: str, base: BaseConfig) -> Path:
    ...
    if exp.waterlevel_geodataset is not None:
        ...
        # selection, so it is another place an extra support point can appear without
        # any other symptom. See model.check_waterlevel_support.
        model.check_waterlevel_support(sf)
...
def collect_metrics(names: list[str]) -> pd.DataFrame:
    ...
    for name in names:
        ...
        if not sealed:
            print(f"[{name}] *** WARNING: not on domain '{dom}' "
...
def main(argv=None) -> int:
    ...
    p.add_argument("--slurm-args", default=None,
                   help="extra sbatch flags, space-separated, e.g. "
                        "'--time=12:00:00 --mem=128G'. sbatch CLI flags beat the "
                        "script's #SBATCH lines, so this is how a big domain gets a "
                        "wall clock without editing the shared batch script. The "
                        "3 h default is NOT enough for v2_barnegat.")
    ...
    for name in names:
        ...
        if args.slurm:
            job = run.submit_slurm(
                exp_dir, sif=str(base.container_sif),
