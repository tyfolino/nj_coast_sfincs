...
# The thresholds and the classifier moved to the DOMAIN REGISTRY
# (nj_sfincs/domain.py). They are per-domain geography: v1's `south_coast` rule
# is `y < 4,458,000` with no lower bound, which is correct on a domain that stops
# at lat 40.15 and silently swallows every Barnegat Bay mark on one that does not.
#
# The rules are still coordinate thresholds, not hand-drawn polygons, and
# domain.V1_MONMOUTH reproduces this module's original classifier EXACTLY (verified
# over 200,000 random points across the v1 footprint: zero disagreements).
#
# Why the split exists at all: pooling ocean-front marks with behind-barrier
# estuary marks hid a completely dammed Shark River Inlet — the estuary marks
# were dry, and dry marks were silently dropped, so the basin reported a
# near-perfect -0.055 m bias while the river behind it never wetted at all.
from .domain import classify_hwm_basin  # noqa: F401,E402


def _basin_names():
    """Basin names for the ACTIVE domain (v1 had a module-level tuple)."""
    from . import domain as _d
    return _d.hwm_basin_names()
