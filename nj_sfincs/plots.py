...
GAUGES = {
    ...
    # ── v2_barnegat: the first INTERIOR gauges that survive the peak ────────────
    # kind="surge", not "tide": unlike Shrewsbury and Shark above — whose records
    # end ~2012-10-29 04:00 and can therefore only be read for pre-storm tide —
    # these two run straight through the crest at 6-min cadence. They are the
    # first chance this project has had to score an interior PEAK rather than
    # infer it from high-water marks.
    #
    # Read them as a PAIR. Mantoloking sits 35 km up the lagoon from the inlet
    # and peaks 0.52 m higher and ~6 h later than Barnegat Light just inside it.
    # A model can match either one alone by getting the overall water level
    # roughly right; matching the difference between them requires the bay's
    # conveyance and inlet exchange to be right.
    "bb_mantoloking": dict(
        kind="surge", lon=-74.05444, lat=40.04056, his="usgs_tidal_bb_mantoloking",
        label="Barnegat Bay at Mantoloking (USGS 01408168)",
        obs=("usgs_sandy_tidal_nj.nc", "waterlevel", 1408168),
        note="mid-lagoon; observed peak 2.11 m NAVD88 @ 2012-10-30 06:18Z",
    ),
    "bb_barnegat_light": dict(
        kind="surge", lon=-74.11056, lat=39.76083, his="usgs_tidal_bb_barnegat_light",
        label="Barnegat Bay at Barnegat Light (USGS 01409125)",
        obs=("usgs_sandy_tidal_nj.nc", "waterlevel", 1409125),
        note="just inside the inlet; observed peak 1.59 m NAVD88 @ 2012-10-30 00:24Z. "
             "Also the check on the artificial 39.70 south boundary 6 km away.",
    ),
}

# Gauges that only exist on some domains. Selecting by the ACTIVE domain's
# observation-point list keeps a v1-domain figure from trying to plot a gauge
# that domain never placed (its his file has no such station).
_GAUGES_ALL = GAUGES


def gauges_for_domain(dom=None) -> dict:
    from . import domain as _d

    dom = dom or _d.active()
    placed = {g.name for g in dom.obs_gauges}
    out = {}
    for key, cfg in _GAUGES_ALL.items():
        his = cfg.get("his")
        # Entries with no explicit `his` are matched by other means downstream;
        # keep them. Entries naming a his point must have that point placed.
        if his is None or his in placed:
            out[key] = cfg
    return out
