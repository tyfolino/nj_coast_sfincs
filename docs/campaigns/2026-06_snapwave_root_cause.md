<!-- Campaign record. Moved out of the Claude memory store 2026-08-05; the
     memory file was `project_snapwave_root_cause`. -->
...
>
> **Standing summary at the time of the move:** NJ Sandy SnapWave+quadtree crash diagnosis — root cause identified 2026-05-26 after a full session of trial-and-error. Boundary input points placed at raw ERA5 grid coords were OUTSIDE the model mesh; SnapWave had no bathymetry there, set depth=0, clamped to 5m, generated runaway waves. Explains ALL prior failures.
...
When we move to X2, revert these X1-and-earlier hacks. **NOTE (2026-06-25):** edits now go DIRECTLY in the hand-maintained notebook `notebooks/sfincs-nj-sandy.ipynb` — `scripts/build_quadtree_notebook.py` is HISTORICAL (do not re-run it; see project_housekeeping_2026_06_25 (memory retired 2026-07-25)). Verify each hack's current state in the notebook before reverting (X1 was already ported in, so some may be gone).
...
## ✅ 2026-07-26 — THE HAZARD MODEL IS CONFIRMED, AND THE DISCRIMINATOR IS **DRY**, NOT "INACTIVE"

The "Active SnapWave cells: matches SFINCS active" row above is now **deliberately false** —
`wave-deep30` ([[project_snapwave_decoupling]]) runs SnapWave on **129,195 faces that are
SFINCS-INACTIVE**, which is the whole point of decoupling, and moves toward Leijnse's row.
**It ran completely clean** — convergence statistically identical to the premier, 0 calls hitting
`niter=100`, no runaway.

⇒ **SnapWave-active + SFINCS-inactive is NOT itself the X1 hazard.** The discriminator is
**DRY / zero-depth**: all 129,195 added faces sit at `snapwavedepth` **8.4–30.0 m (median 18.9),
zero at or below 0.5 m**. That is why `create_active(zmin=-30)` was dangerous (it swept in 10,431
**inland** cells) while the decoupled mask is safe — same "inactive" label, opposite depth.
**Check depth, not the mask flag.** ⚠️ And check it with `snapwavedepth`: `zb` is NaN on
SFINCS-inactive faces, so a `zb` test returns 0 matches and looks clean (see
[[reference_build_traps]] §4).
...
Related: project_quadtree_session (memory retired 2026-07-25), project_snapwave_plan (memory retired 2026-07-25), [[project_compound_roadmap]], project_stockdon_setup (memory retired 2026-07-25).
