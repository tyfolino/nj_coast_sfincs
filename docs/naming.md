...
This repo hosts several domains, so **every run lives at `experiments/<domain>/<arm>`** —
see "Domain namespacing" at the bottom. That rule was written here on 2026-07-27 but only
implemented in code on 2026-08-05, when the v1 and v2 repos were consolidated; until then
both repos had a flat `experiments/` and each held a *different* model under the names
`faber-waves-premier` and `_template_sealed`.

> Written when this file lived in the frozen `nj_sandy_sfincs` repo, whose domain was
> `v1_monmouth` throughout. Where the text below says "this repo is frozen", read
> "`v1_monmouth` is frozen" — the domain is, the repo no longer exists separately.
...
the worst **extent** model — see `docs/campaigns/2026-07_snapwave_decoupling.md` for the trade.

## Domain namespacing

Names are `<domain>/<arm>` using the `DOMAINS` registry keys verbatim (the first token IS
the `NJ_DOMAIN` value). `nj_sfincs/config.py:exp_root()` resolves this, so setting
`NJ_DOMAIN` selects the run tree, the geography and the fingerprint check in one move:
